...
if !exists(global.heaterfault_timer)
  ...
  if global.heaterfault_timer >= 0
    ...
    if var.timeout == 0
      M291 R{var.error_title} P"Printer was shut down because of a heater fault." S2
      M81 ; turn power off immediately
    else
      M291 P{"Heater error detected — the printer will shut down in "^var.timeout^" seconds."} R{var.error_title} S4 K{"Shut down now","Keep power on"} J2 T{var.timeout}
      if result == -1 || input == 0
    ...
    echo "Error: The 'heaterfault_timer' value is negative: " ^ global.heaterfault_timer;
