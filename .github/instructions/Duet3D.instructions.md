if something is not clear before referring to codes search in instruction file to make sure you read the documentation correctly. 


\" dont use, use "" instead

The maximum length of a G-code command line for Duet3D, including the command itself and all parameters, is 256 characters. This limit applies to the entire line of text as processed by the Duet firmware. 


M291 T0 - lets you keep notification indefinitely
...
### **dimmwhite.g** - Dimming White LEDs
- **Purpose**: Gradually dims white LEDs from current brightness to off
- **Method**: Uses fan output P6 to control white LEDs, decreasing brightness in steps
- **Effect**: Smooth fade-out of white chamber lighting

### **end.g** - Green Status Light
- **Purpose**: Print completion indicator - turns on green lights
- **Sequence**: Turns off status LEDs → dims white LEDs → gradually brightens green (P2)
- **Effect**: Solid green lighting to indicate successful print completion

### **fault.g** - Red Error Light  
- **Purpose**: Error/fault indicator - turns on red lights
- **Sequence**: Turns off status LEDs → dims white LEDs → gradually brightens red (P1)
- **Effect**: Solid red lighting to indicate system fault or error condition

### **pause.g** - Yellow Warning Light
- **Purpose**: Pause/attention indicator - creates yellow lights
- **Method**: Combines red (P1) at full brightness + green (P2) at reduced brightness (÷2.5)
- **Effect**: Yellow/amber lighting to indicate paused state or warning

### **red.g** - Red Flash
- **Purpose**: Basic red light activation (used for alerts/errors)
- **Method**: Gradually brightens red LED (P1) from 0 to full brightness
- **Effect**: Red lighting that ramps up smoothly

### **resetstatus.g** - Reset to Normal
- **Purpose**: Returns LEDs to normal white operational state
- **Sequence**: Turns off all colored status LEDs → restores white lighting
- **Effect**: Back to standard white chamber illumination

### **restorewhite.g** - White Light Restoration
- **Purpose**: Gradually brings white LEDs back to their previous brightness level
- **Method**: Ramps up fan P6 (white LEDs) from current value to maximum
- **Effect**: Smooth fade-in of white chamber lighting

### **resume.g** - Resume Normal Operation
- **Purpose**: Returns to normal state after pause/error
- **Method**: Simply calls resetstatus.g
- **Effect**: Restores standard white lighting (resume normal operation)

### **start_cold.g** - Cold Start Blue Light
- **Purpose**: Indicates heating phase - printer is cold, starting to heat
- **Method**: Combines blue (P3) + reduced green (P2÷1.7) for cyan-blue effect
- **Effect**: Light blue/cyan lighting during initial heating phase

### **start_hot.g** - Hot Start Sequence
- **Purpose**: Indicates transition from heating to ready state
- **Sequence**: 
  1. Dims blue lights (heating complete)
  2. Flashes red briefly (temperature reached)  
  3. Returns to white lighting (ready to print)
- **Effect**: Visual sequence showing heating completion

### **startup.g** - System Startup Sequence
- **Purpose**: Boot-up light show
- **Sequence**:
  1. Red flash (system initializing)
  2. Red fade out
  3. White lights fade in (system ready)
- **Effect**: Startup animation from red to white

### **statusoff.g** - Turn Off Status LEDs
- **Purpose**: Utility to turn off all colored status indicators
- **Method**: Sets all status LEDs (P1=red, P2=green, P3=blue) to 0
- **Effect**: Clears all colored status lighting

### **stop.g** - Stop/Reset State
- **Purpose**: Print stop indicator
- **Method**: Calls resetstatus.g to return to normal white lighting
- **Effect**: Returns to standard white lighting after print stop

## **LED Port Mapping Summary:**
- **P1** = Red LEDs
- **P2** = Green LEDs  
- **P3** = Blue LEDs
- **P6** = White LEDs (fan-controlled)

## **Color Combinations:**
- **Red** = Error/Fault/Alert
- **Green** = Success/Complete
- **Yellow** (Red+Green) = Pause/Warning
- **Blue/Cyan** = Heating/Cold Start
...
Note echo only echos back to the input its sent from, or to a file. To get information to a different input (e.g. echo to the HTTP UI from a macro called from a different input) use M118.
...
If a while loop is used in a GCode job file (not in a macro), it should have M400 at the end, and immediately before any uses of 'continue' within the loop.
...
1 In RRF 3.5.0-rc3 and earlier, pressing 'Cancel' or M291 timing out will cancel the current job/macro, and any subsequent lines in the job/macro are not processed, rather than set result to -1. See this discussion on the Duet3D forums for a workaround. Note that DSF 3.5 and 3.6 do not update the result constant when a G-code is interpreted by DSF or third-party plugins.
Literals
...
Expressions may use the values of any properties in the RepRapFirmware Object Model (OM). See Object Model of RepRapFirmware to see what is available.
Variables
...
daemon.g
The macro file /sys/daemon.g can be used to execute regular tasks. The firmware looks for the file, if the file exists it executes it and once the end of file is reached it waits. If the file is not found it waits for 10s and then looks for it again.
...
In SBC mode only, DSF waits for pending codes to be executed before a meta code is evaluated. M576 S0 is likely to help, to reduce the delay between SPI transfers. This will be addressed in RRF 3.7, see this Github issue.
Job information and custom Object Model keys
RepRapFirmware extracts data from the currently-running Gcode job when it runs, and stores it in the Object Model. From RRF 3.6.0, user-defined Object Model keys and values can be embedded in the Gcode file, which will be created in the OM when the Gcode file is run. See Object Model documentation.
Examples of use
Some examples using meta Gcode are listed below. There are also many examples and discussions on the meta Gcode section of the forum.
...
Sent to over USB (for example from Pronterface)
Sent by the Duet Web Control (DWC) Interface
...
Connecting hobby servos and DC motors
Controlling unused IO pins
...
;So is this
G28
In CNC mode only (M453), comments can be enclosed in a single pair of parentheses, e.g. (comment). Comments cannot include double or nested parentheses, e.g. (comment (a bit more comment)), and they must start and end on the same line. This complies with NIST Gcode interpreter guidelines.
...
See SD card for a full explanation of the SD card structure.
Live editing
...
Create a valid probe points file; the default file name is /sys/probePoints.csv. The format of a probe points file is similar to a height map file except for the following:
...
Send an appropriate G29 S4 command, eg G29 S4 P"probePoints.csv".
When G29 S0 is called subsequently, the grid definition defined in the probe points file is used instead of the grid defined by M557, and reachable points are probed or not as indicated in the file.
G30: Single Z-Probe
Usage
...
All motors and heaters are are turned off.
...
See Firmware configuration limits for the number of inputs and outputs each Duet board and firmware version supports.
See also
M280, M950
...
Also used to set the idle timeout for all motors. For example, M84 S10 will idle the stepper motors after 10 seconds of inactivity. The idle current is set by the M906 I parameter.
Notes
...
See Firmware configuration limits for the number of bed heaters each Duet board and firmware version supports.
...
See Firmware configuration limits for the number of chamber heaters each Duet board and firmware version supports.
...
S4: Display a number of choices. The names of the choices are given by the K parameter as an array of strings. The choice selected by the user, as the array index, is available to be used in the "input" constant (blocking, RRF 3.5 and later only)
S5: Prompt for an integer value. L is the minimum accepted value (default 0), H is the maximum accepted value (default unlimited), and F is the default value. The integer is available to be used in the "input" constant (blocking, RRF 3.5 and later only)
S6: Prompt for a floating point value. L is the minimum accepted value (default 0.0), H is the maximum accepted value, and F is the default value. The floating point value is available to be used in the "input" constant (blocking, RRF 3.5 and later only)
S7: Prompt for a string value. L is the minimum number of characters (default 1), H is the maximum number of characters (default 10), and F is the default value. The string is available to be used in the "input" constant (blocking, RRF 3.5 and later only)
Notes
...
Fnn (RRF 3.3 and later) Fan PWM to use when the print cooling fan is turned on (ignored if the T parameter is not present), default 0.7 in RRF 3.4.0. Use a lower value if your printer uses a powerful print cooling fan that you do not normally run at full PWM.
...
This spreadsheet can help to understand the U and V parameters for Linear Temperature Compensation
...
See Firmware configuration limits for the number of probe points each Duet board and firmware version supports. Can be 961 (31x31 grid) or 441 (21x21 grid). In RRF 1.x - 121 on the Duet 06/085 (enough for a 11x11 grid).
...
Dnn (firmware 2.0 and later, only applies to TMC2660, TMC22xx, TMC2160 and TMC5160 stepper drivers) Driver mode: 0=constant off time, 1=random off time (TMC2660 drivers only), 2=spread cycle, 3=stealthChop (mode 3 for TMC22xx/TMC2160/TMC5160 only), 4=Closed Loop, 5=Assisted open loop (modes 4 and 5 are only for Duet 3 closed loop controllers and motors such as the 1HCL and the Motor23CL). The default is spreadCycle for all drivers from RRF 3.4, and stealthChop2 for TMC22xx in RRF 3.3 and earlier. In stealthChop mode the drivers will switch over to spreadCycle automatically at high speeds, see the V parameter.
...
See Firmware configuration limits for the number of triggers each Duet board and firmware version supports.
...
For more information on setting up bed levelling, see Bed levelling using multiple independent Z motors
...
Qnnn (RRF 3.3 and later) SPI clock frequency (optional, default 2000000 i.e. 2MHz)
Examples
...
Notes
The P parameter selects which accelerometer to use and is mandatory. To use an accelerometer on a CAN-connected expansion board, use the form Pboard-address.device-number for example P22.0. Use P0 for an accelerometer connected locally (i.e. on the mainboard) via SPI.
...
To use an accelerometer on a CAN-connected expansion board, use the form Pboard-address.device-number for example P22.0.
M957: Raise event
...
If the new tool number is not the same as the old tool number, including if either old or new tool number is -1 (i.e. no tool), the current coordinates are saved to memory slot 2 automatically (see G60).
Run macro tfree#.g where # is the number of the old tool.
If a tool is already selected, deselect it and set its heaters to their standby temperatures (as defined by the R parameter in the most recent G10/M568 command for that tool)
...
This field is obsolete and will be removed in the future: Use RSSI instead This field is exclusively maintained by DSF in SBC mode and/or by DWC. It is not available in standalone mode
